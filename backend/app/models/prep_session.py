...
class PrepSession(Base):
    ...
    document_id = Column(
        String, ForeignKey("documents.id"), nullable=False, unique=True, index=True
    )  # One prep session per document
    user_id = Column(String, ForeignKey("users.id"), nullable=False, index=True)
    title = Column(String, nullable=True)  # Optional user-defined title
    status = Column(
        String, nullable=False, default="preparing", index=True
    )  # preparing, ready, archived
    ...
    interview_sessions = relationship(
        "InterviewSession", back_populates="prep_session", cascade="all, delete-orphan"
    )

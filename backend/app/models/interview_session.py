...
class InterviewSession(Base):
    ...
    project = relationship("Project", back_populates="interview_sessions", foreign_keys=[project_id])
    stakeholder_profile = relationship("StakeholderProfile", back_populates="interview_sessions", foreign_keys=[stakeholder_profile_id])
    ...
    live_utterances = relationship("LiveUtterance", back_populates="session", cascade="all, delete-orphan")
    transcript_revisions = relationship("TranscriptRevision", back_populates="session", foreign_keys="[TranscriptRevision.session_id]", cascade="all, delete-orphan")
    final_utterances = relationship("FinalUtterance", back_populates="session", cascade="all, delete-orphan")
    ai_usage_events = relationship("AIUsageEvent", back_populates="interview_session", cascade="all, delete-orphan")
    interview_brief = relationship("InterviewBrief", back_populates="session", uselist=False, cascade="all, delete-orphan")
    insight_memo = relationship("InterviewInsightMemo", back_populates="session", uselist=False, cascade="all, delete-orphan")

...
class StakeholderCardGenerator:
    ...
    def _generate_themes(
        ...
        try:
            ...
            system_prompt = f"""你是 BRD 需求訪談規劃專家。你幫助商業分析師設計針對特定利害關係人的訪談主題，目的是填補 BRD 的資訊缺口。

產生 {theme_count_hint} 個訪談主題。總時長控制在 {duration} 分鐘內。

嚴格規則：
- 每個主題要對準這位受訪者的第一手經驗、職責或決策權限，避免任何人都能回答的通用主題。
- 不要產生「開場」「範圍確認」「結尾」等訪談技巧類主題 — 這些由系統自動處理
- 主題應該對應可寫入 BRD 的具體內容區塊（流程、痛點、需求、限制等）
- 如果受訪者有不熟悉的領域，相關主題不要出現
- rationale 要用「了解」「釐清」「確認」等中性動詞說明提問目的，不要使用「只有他／他們能」「唯有此人」或其他排他式句型
...
主題說明：{theme.rationale}"""
        ...
        try:
            ...
            system_prompt = """你是 BRD 訪談問題設計專家。為特定受訪者設計具體的訪談問題。問題的用語和深度要配合受訪者的專業背景。

嚴格規則：
- 每個問題必須是「需要受訪者提供具體資訊」的問題
- 根據受訪者的角色類型調整問題深度：
  • 業務使用者：用白話、從日常經驗出發（「上次遇到...」「通常怎麼處理...」）
  • 技術人員：可用技術術語、問架構細節
  • 管理者：問決策標準、優先級、資源限制
- 不要產生以下類型的問題：
  • 開場白（「您好，我想先請教...」）

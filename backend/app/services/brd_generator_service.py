...
class BRDGeneratorService:
    ...
    def generate_brd(self, db: Session, session_id: str) -> BRDDraft:
        ...
        if session.status not in ["ended", "paused"]:
            raise ValueError(f"Interview session must be ended or paused to generate BRD. Current status: {session.status}")
        ...
        try:
            ...
            brd_draft.generation_duration_seconds = str(int((end_time - start_time).total_seconds()))
    ...
    def _extract_interview_data(self, db: Session, session: InterviewSession) -> Dict[str, Any]:
        ...
        card_states = db.query(InterviewCardState).filter(
            InterviewCardState.session_id == session.id
        ).all()
...
Return your response as a JSON object with these fields."""
                ...
                response_format={
                    ...
                    "json_schema": {
                        ...
                        "schema": {
                            ...
                            "required": [
                                "title", "executive_summary", "project_overview",
                                "business_objectives", "success_criteria", "stakeholders",
    ...
    def _extract_requirements(self, db: Session, interview_data: Dict[str, Any]) -> List[Dict[str, Any]]:
...
Return a JSON array of requirements."""
                                    ...
                                    "requirements": {
                                        ...
                                        "items": {
                                            ...
                                            "properties": {
                                                ...
                                                "type": {"type": "string", "enum": ["functional", "non_functional", "business", "user", "technical"]},
                                            ...
                                            "required": ["title", "description", "type", "priority", "user_story", "acceptance_criteria"],
                                            "additionalProperties": False
                                        }
                                    }
                                },
    ...
    def _create_requirements(self, db: Session, brd_draft_id: str, requirements_data: List[Dict[str, Any]]):
    ...
    def _generate_markdown(self, brd_draft: BRDDraft, requirements_data: List[Dict[str, Any]]) -> str:

...
from app.services.prep_session_service import prep_session_service, PrepSessionService
...
class TestPrepSessionService:
    ...
    def test_delete_prep_session_success(
        self,
        mock_doc_service,
        mock_db,
        sample_prep_session
        ...
        mock_doc_service.delete_document.assert_called_once_with(
            mock_db,
            "doc-123",
            commit=False
        )
    ...
    def test_list_prep_sessions_basic(self, mock_db):
        ...
        with patch('app.services.prep_session_service.billing_service') as mock_billing:
            mock_billing.summarize_decks.return_value = {
                "doc-123": {
                    "totalCostUsd": 1.50,
                    "inputTokens": 1000,
                    "outputTokens": 500
    ...
    def test_get_prep_session_presentation_sessions(self, mock_db, sample_prep_session):
        ...
        result = prep_session_service.get_prep_session_presentation_sessions(
            db=mock_db,
            prep_session_id="prep-123"
        )
    ...
    def test_get_prep_session_presentation_sessions_empty(self, mock_db, sample_prep_session):
    ...
    def test_get_prep_session_presentation_sessions_not_found(self, mock_db):
        """Test getting sessions for nonexistent prep session."""
        mock_db.query().filter().first.return_value = None

        with pytest.raises(HTTPException) as exc_info:
            prep_session_service.get_prep_session_presentation_sessions(
                db=mock_db,
                prep_session_id="nonexistent"
            )

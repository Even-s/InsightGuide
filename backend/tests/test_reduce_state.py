...
class TestReduceState:
    ...
    def test_zero_confidence_stays_pending(self):
        state, _act, _comp = self.engine._reduce_state("pending", self._judgment(confidence=0))
    ...
    def test_low_completion_from_answer_creates_progress(self):
        ...
        state, _act, _comp = self.engine._reduce_state("pending", self._judgment(confidence=0.1))
    ...
    def test_medium_confidence_probably_sufficient(self):
        state, _act, _comp = self.engine._reduce_state("pending", self._judgment(confidence=0.5))
        assert state == "probably_sufficient"
    ...
    def test_cannot_go_back_from_listening_to_pending(self):
        state, _act, _comp = self.engine._reduce_state("listening", self._judgment(confidence=0))
        assert state == "listening"
    ...
    def test_cannot_go_back_from_sufficient(self):
        state, _act, _comp = self.engine._reduce_state("sufficient", self._judgment(confidence=0))
        assert state == "sufficient"
    ...
    def test_low_completion_with_response_is_probably_sufficient(self):
        ...
        state, _act, _comp = self.engine._reduce_state("pending", self._judgment(confidence=0.25))
        assert state == "probably_sufficient"

    def test_higher_completion_still_probably_sufficient(self):
        state, _act, _comp = self.engine._reduce_state("pending", self._judgment(confidence=0.26))
        assert state == "probably_sufficient"

...
**Current Focus**: 🔴 **Code Cleanup (Critical Priority)**  
**Target Launch**: ~9 weeks (August 2026)

**Quick Links**:
- 📋 [**QUICK_STATUS.md**](./QUICK_STATUS.md) - At-a-glance status (start here!)
...
- 🧹 [**CLEANUP_STATUS.md**](./CLEANUP_STATUS.md) - Legacy code cleanup status
...
- `./insightguide.sh launch`：需要時啟動服務，完成後開啟 `http://localhost:5173`。
...
### 完成的階段
- ✅ **Phase 1**: 專案設定與品牌識別 (100%)
- ✅ **Phase 2**: 後端資料模型與 API (100%)
- ✅ **Phase 3**: 前端組件調整 (100%)
- ✅ **Phase 4**: AI 服務調整 (100%)
...
- **總任務數**: 30/30 (100%)
- **建立檔案**: 50+ 個新檔案
- **API Endpoints**: 36 個新 endpoints
- **前端組件**: 16 個新組件
- **詳細文檔**: 查看 `IMPLEMENTATION_SUMMARY.md`
...
完整架構文件請參考 [`docs/architecture/InsightGuide_開發架構書.md`](docs/architecture/InsightGuide_開發架構書.md)。
...
2. 執行資料庫 migrations
3. 啟動後端 API（port 8001）
4. 啟動 Celery worker（背景任務）
5. 啟動前端 dev server（port 5173）
...
所有文件已整理至 [`docs/`](docs/) 目錄：

- **[架構](docs/architecture/)** - 系統架構與技術規格
- **[里程碑](docs/milestones/)** - 里程碑計畫、實作與完成報告
- **[指南](docs/guides/)** - 開發指南、quickstart 與測試指南
